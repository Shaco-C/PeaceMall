...
public class ShopClientFallbackFactory implements FallbackFactory<ShopClient> {
    ...
    public ShopClient create(Throwable cause) {
        return new ShopClient() {
            ...
            @Override
            public PageDTO<ShopDTO> findAllShopsWithPage(int page, int pageSize) {
                log.error("调用shop服务查询商店基本信息失败:{}",page,pageSize,cause);
                throw new RuntimeException("调用shop服务查询商店基本信息失败");
            }
        };
    }
}
